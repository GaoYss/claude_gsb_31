...
3. **完成时间统一口径**：`completed_at` 一律为「业务完成日期」的本地零点（`YYYY-MM-DD 00:00:00`）——有养护记录的任务取最新作业日期，无记录的任务取操作当天，不再写入 UTC 当前时刻，与看板按业务日期归月的统计保持一致；该口径只约束新写入的数据，不回填历史完成时间，已封账月份的统计不受影响。
4. **绿地归属一致性**：养护记录可只填绿地（日常养护）或只填任务（绿地自动跟随任务）；两者同时提供时必须属于同一绿地。更换记录若关联养护记录，必须是同一绿地的记录。
5. **日期约束**：养护日期、更换日期不得早于绿地建成日期。
6. **金额核算**：更换金额 = 数量 × 单价，由后端统一计算；未填单价时金额留空，前端提示补录。
7. **删除保护**：删除绿地时若已存在任务/记录/更换数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失。
