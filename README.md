...
8. **完成时间口径**：`completed_at` 统一为日期粒度（当天 `00:00:00`），不再写入具体时分秒。有养护记录联动完成的任务以**最新养护日期**为准；无记录手动标记完成的以**操作当天**为准。同一任务无论走哪条路径、在当天哪个时刻操作，归属月份唯一确定。该口径只作用于变更后新发生的流转，历史数据不回填、不改写，已封账月份的统计不受影响。
